...
type TabKey = 'pauschalreisen' | 'mietwagen'
...
/**
 * Comparatore CHECK24 con le due schede (Pauschalreisen / Mietwagen).
 * È il billboard orizzontale che stava dentro la hero fino al commit 4b5ff2a:
 * stessi widget, stessi fix su datepicker e fullscreen mobile.
 */
export function Check24SearchTabs() {
  ...
  // Carica subito anche il Mietwagen: i due widget stanno impilati nella stessa
  // cella di griglia, quindi la card prende sempre l'altezza del piu' alto e
  // cambiando scheda non salta nulla.
  useEffect(() => {
    loadMietwagenWidget()
  }, [loadMietwagenWidget])
  ...
  return (
    ...
    <div className="w-full max-w-5xl 2xl:max-w-6xl mx-auto">
      ...
      >
        ...
        {/* I due widget condividono la stessa cella di griglia: la card e' sempre
            alta quanto il piu' alto dei due, cosi' cambiando scheda non cambia misura */}
        <div className={`grid ${fullscreen ? 'flex-1 overflow-y-auto' : ''}`}>
          <div
            className={`c24-hero-widget col-start-1 row-start-1 flex flex-col justify-center transition-opacity ${activeTab === 'pauschalreisen' ? 'opacity-100' : 'invisible opacity-0 pointer-events-none'}`}
            aria-hidden={activeTab !== 'pauschalreisen'}
          >
          ...
          <div
            className={`c24-mietwagen-widget col-start-1 row-start-1 flex flex-col justify-center transition-opacity ${activeTab === 'mietwagen' ? 'opacity-100' : 'invisible opacity-0 pointer-events-none'}`}
            aria-hidden={activeTab !== 'mietwagen'}
          >
