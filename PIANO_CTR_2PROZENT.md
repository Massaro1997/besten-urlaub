...
| 2026-07-07 | Ralph loop 10 giri (Fable) | FASI 2+3+4 ESEGUITE IN UNA SESSIONE: (G1) `TopSeitenLinks` su home+hub /reise+/fragen verso le 10 pagine top GSC (`src/data/top-seiten.ts`, aggiornare a ogni check); (G2) `dateModified=BUILD_DATE` (era congelato 16/06); (G3) hub /reise sezione "Wohin im [mese+1]" ricalcolata a ogni build; (G4) contenuto unico manuale su 10 pagine top via overlay `src/data/seiten-extra.ts` (chiave `ziel/monat` o slug fragen); (G6-8) NUOVO ASSE `/pauschalreise/[ziel]` 58 pagine + hub: Preistabelle 12 mesi con link ai mesi, Product+AggregateOffer schema, FAQ, prose data-driven, **forge 58/58 PASS** cluster unico; sitemap +59 URL prio 0.7-0.8; cross-link 4 direzioni (reise-monat CTA, fragen related, reiseziel intro, pauschalreise→fragen guardati con getFrage per NEAR_DUP_BLOCK); (G9) home title keyword-first senza doppio brand ("Pauschalreisen günstig buchen...", era "Bester Urlaub — Die besten..."), link reiseziel→pauschalreise. Build 1161 pagine PASS. | 0,16% |
